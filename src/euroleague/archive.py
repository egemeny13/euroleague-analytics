...
def restore_current_season_cache(
    ...
    *,
    ...
    The optional snapshot receives the same already-downloaded, checksum-verified
    bodies before the canonical directory is swapped. A parser can therefore
    consume that private directory even if another process later advances or
    restores the canonical cache while the run is still working.
    """
