...
| 5 | **In progress — repair built, production write not run:** [`04-e2024-points-archive-repair.md`](docs/superpowers/plans/2026-08-23-04-e2024-points-archive-repair.md) | Close the known recoverability hole without re-fetching source data. The cache blocker is gone: the 330 E2024 `Points` bodies were carried to this machine on 2026-08-25 and verified against their transport manifest. Source re-fetch is still not an approved substitute. | All 330 objects and index rows verify and reconciliation is clean. |
...
Orders 1-4 are complete. Order 5's cache blocker is resolved and its gate is
unchanged and not yet met: the owner approved starting Order 6 before Order 5 on
2026-08-24, without weakening or waiving that gate. Order 6 is complete through
reviewed production activation and an unchanged idempotency rerun. Order 5 now
has a verified local cache — 330 `Points` bodies, 16,713,709 exact bytes,
51,193 coordinate rows matching the 51,193 E2024 `raw_shot` rows — and an
endpoint-scoped, resumable repair path with per-object verification, proved
offline. Its Storage and database writes have not run, because this machine
holds no credentials and the plan requires owner approval immediately before
the first write; evidence is in `docs/E2024_POINTS_ARCHIVE_REPAIR_REPORT.md`.
