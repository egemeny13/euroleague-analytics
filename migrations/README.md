...
| `0020_tester_role` | Adds `el_tester`, the login role a human tester is given: the same `select` reach as `el_reader` - the seven views and the twelve base tables they read - plus `bypassrls`, and nothing else. Creates no table and no view. Separate from `el_reader` because that one is the hosted server's credential and revoking a leaked tester copy of it would interrupt production. The migration sets **no password**; the owner sets it separately so it never enters version control. Approved by Decision 43. **Not yet rehearsed and not applied** - it still needs the up/down/up/down gate on a disposable PostgreSQL and the owner's approval immediately before the production apply. |
